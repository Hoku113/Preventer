...
class PoseEstimator():
    ...
    def run(self):
        self.frame, self._before_3d_frame = self.predict()

    def preprocessed(self, frame, size, flip, interpolation):
        self.frame = frame
        if size is not None:
            self.frame = cv2.resize(self.frame, size, interpolation=interpolation)
        if flip:
    ...
    def predict(self):
        ...
        scaled_img = cv2.resize(self.frame, dsize=(self._model.inputs[0].shape[3], self._model.inputs[0].shape[2]))
        img = scaled_img[
        ...
        if self._fx < 0:
            self._fx = np.float32(0.8 * self.frame.shape[1])
        ...
        if len(poses_3d):
            ...
            # 転倒危険者を判定
            danger_person_index = self._submit_joint(poses_3d)
            if danger_person_index == []:
                pass
            else:
                self._draw_dangerous_person(poses_2d, scaled_img, danger_person_index)
                # send_blob(self.frame)
        ...
        self._draw_poses(poses_2d, scaled_img)
        return self.frame, poses_3d

    def _draw_dangerous_person(self, poses_2d, scaled_img, danger_person_index):
        for index in danger_person_index:
            pose = np.array(poses_2d[index][0: -1]).reshape((-1, 3)).transpose()
            pose[0], pose[1] = (
                pose[0] * self.frame.shape[1] / scaled_img.shape[1],
                pose[1] * self.frame.shape[0] / scaled_img.shape[0]
            )
    ...
    def _draw_poses(self, poses_2d, scaled_img):
        for pose in poses_2d:
            ...
            for edge in BODY_2D_EDGES:
                if was_found[edge[0]] and was_found[edge[1]]:
                    cv2.line(self.frame, tuple(pose[0:2, edge[0]].astype(np.int32)),
    ...
    def _submit_joint(self, poses_3d):
        index = 0
        danger_person_index = []

        # 初回フレームのみ有効
        if self._before_3d_frame is None:
            return danger_person_index
        else:
            try:
                for x in range(len(poses_3d)):
                    if len(poses_3d) != len(self._before_3d_frame):
                        raise Exception("人数が変わりました")

                    submit = np.abs(self._before_3d_frame[x] - poses_3d[x])
                    submit = np.sum(submit)
                    np.set_printoptions(precision=1, suppress=True)
                    submit_total = int(submit)
                    if submit_total >= 5000:
                        pass
                    elif submit_total >= 10:
